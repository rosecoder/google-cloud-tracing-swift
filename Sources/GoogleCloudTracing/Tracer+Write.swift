...
extension GoogleCloudTracer {
  ...
  private func _write(spans: [Span]) async throws {
    ...
    _ = try await client.batchWriteSpans(
      .with {
  ...
  private func encode(span: Span, projectID: String) -> Google_Devtools_Cloudtrace_V2_Span? {
    ...
    return .with {
      $0.name =
        "projects/\(projectID)/traces/\(trace.id.prefixedHexRepresentation)/spans/\(spanIDString)"
      ...
      $0.displayName = Google_Devtools_Cloudtrace_V2_TruncatableString(
        span.operationName, limit: 128)
  ...
  private func encode(attributes: SpanAttributes, context: ServiceContext)
    -> Google_Devtools_Cloudtrace_V2_Span.Attributes
  {
    ...
    attributes.forEach { key, value in
      ...
      case .stringConvertible(let value):
        encodedValue = .with {
          $0.stringValue = Google_Devtools_Cloudtrace_V2_TruncatableString(
            value.description, limit: 256)
        }
      default:
  ...
  private func encode(event: SpanEvent, context: ServiceContext)
    -> Google_Devtools_Cloudtrace_V2_Span.TimeEvent
  {
    return .with {
