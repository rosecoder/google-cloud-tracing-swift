...
public final class Span: Tracing.Span {
  ...
  private let onEnd:
    @Sendable (GoogleCloudTracer.Span, _ endTimeNanosecondsSinceEpoch: UInt64) -> Void
  ...
  init(
    ...
    onEnd: @escaping @Sendable (GoogleCloudTracer.Span, _ endTimeNanosecondsSinceEpoch: UInt64) ->
      Void
  ) {
    self.id = id
